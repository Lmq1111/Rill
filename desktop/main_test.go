...
// can leak into the developer's real Rill directories.
func TestMain(m *testing.M) {
	dir, err := os.MkdirTemp("", "rill-desktop-test")
	if err != nil {
		os.Exit(1)
	}
	os.Setenv("HOME", dir)
	os.Setenv("RILLAGENT_CREDENTIALS_STORE", "file")
	os.Setenv("USERPROFILE", dir)
	os.Setenv("XDG_CONFIG_HOME", dir+"/config")
	os.Setenv("RILLAGENT_STATE_HOME", dir+"/state")
	os.Setenv("RILLAGENT_CACHE_HOME", dir+"/cache")
