...
func (a *App) startTray() bool {
	...
	a.mu.Unlock()
		...
		systray.SetTitle(brand.ProductName)
		systray.SetTooltip(brand.ProductName)
...
func trayMenuLabels(locale string) trayLabels {
	if locale == "zh" {
		return trayLabels{
			openTitle:   "打开",
			openTooltip: "打开 " + brand.ProductName + " 窗口",
			quitTitle:   "退出",
			quitTooltip: "退出 " + brand.ProductName,
		}
	}
	return trayLabels{
		openTitle:   "Open",
		openTooltip: "Open the " + brand.ProductName + " window",
		quitTitle:   "Quit",
		quitTooltip: "Quit " + brand.ProductName,
	}
}
