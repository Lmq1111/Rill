...
func TestDiagnoseBotConnectionBuildsReportDetailForMissingSecret(t *testing.T) {
	...
	if _, err := app.upsertBotConnection(config.BotConnectionConfig{
		...
		Enabled:       true,
		Status:        "connected",
		WorkspaceRoot: "/Users/alice/work/rillagent",
...
func TestRememberBotConnectionRemoteStoresStableScope(t *testing.T) {
	...
	if _, err := app.upsertBotConnection(config.BotConnectionConfig{
		...
		Enabled:       true,
		Status:        "connected",
		WorkspaceRoot: "/tmp/rillagent-project",
	}, nil); err != nil {
	...
	if projectMapping.Scope != "project" || projectMapping.WorkspaceRoot != "/tmp/rillagent-project" || projectMapping.RemoteID != "wxid_project" {
