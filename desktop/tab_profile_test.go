...
effort = "max"
`
	if err := os.WriteFile(filepath.Join(projectRoot, "rillagent.toml"), []byte(configBody), 0o644); err != nil {
		t.Fatal(err)
	}
...
`
	if err := os.WriteFile(filepath.Join(root, "rillagent.toml"), []byte(configBody), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestAutoResearchStatusSurfaceForActiveTab(t *testing.T) {
	...
	heartbeatPath := filepath.Join(root, ".rillagent", "autoresearch", current.TaskID, "logs", "heartbeat.jsonl")
...
func TestAutoResearchFindingsAreLoadedOnDemand(t *testing.T) {
	...
	findingsPath := filepath.Join(root, ".rillagent", "autoresearch", current.TaskID, "state", "findings.jsonl")
...
func userConfigPathForTest() string {
	if dir, err := os.UserConfigDir(); err == nil {
		return dir + "/reasonix/rillagent.toml"
	}
	return ""
}
