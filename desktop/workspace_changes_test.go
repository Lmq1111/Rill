...
func TestWorkspaceGitBranchForMetaDoesNotBlockOnColdProbe(t *testing.T) {
	...
	start := time.Now()
	if got := workspaceGitBranchForMeta("/tmp/rillagent-cold-probe"); got != "" {
	...
	releaseProbe()
	eventuallyBranchForMeta(t, "/tmp/rillagent-cold-probe", "feature/async")
}

func TestWorkspaceGitBranchForMetaReturnsStaleDuringRefresh(t *testing.T) {
	resetWorkspaceGitBranchMetaCacheForTest(t)
	workspaceGitBranchCache.Lock()
	workspaceGitBranchCache.entries[filepath.Clean("/tmp/rillagent-stale-probe")] = workspaceGitBranchCacheEntry{
	...
	start := time.Now()
	if got := workspaceGitBranchForMeta("/tmp/rillagent-stale-probe"); got != "feature/stale" {
	...
	releaseProbe()
	eventuallyBranchForMeta(t, "/tmp/rillagent-stale-probe", "feature/fresh")
}
