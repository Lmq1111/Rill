// Command windows-resource stamps Rill branding and metadata into a Windows
...
const (
	companyName = "Rill"
	copyright   = "Copyright © 2026 Rill Contributors"
)
...
func stampExecutable(opts resourceOptions) error {
	...
	for key, value := range map[string]string{
		...
		version.ProductName:      "Rill",
		version.ProductVersion:   opts.numericVersion,
		version.Comments:         "Rill desktop support component.",
	} {
...
func replaceFile(path string, data []byte, mode os.FileMode) error {
	temp, err := os.CreateTemp(filepath.Dir(path), ".rillagent-resource-*.exe")
	if err != nil {
		return err
	}
	tempName := temp.Name()
