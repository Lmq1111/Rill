...
func TestStampExecutableSupportsWindowsReleaseArchitectures(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.arch, func(t *testing.T) {
			...
			executable := filepath.Join(dir, "rill-launcher.exe")
			...
			opts := resourceOptions{
				...
				fileDescription: "Rill Launcher",
				internalName:    "rill-launcher",
				originalName:    "rill-launcher.exe",
			}
			...
			if got := (*info.Table()[version.LangDefault])[version.OriginalFilename]; got != "rill-launcher.exe" {
