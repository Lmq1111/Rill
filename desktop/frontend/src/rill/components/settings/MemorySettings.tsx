...
import { Section, StateSwitcher, ConfirmDialog, Drawer, Select, Row, Toggle } from "./kit";
...
export function MemorySettings() {
  const { params, projects: liveProjects } = useStore();
  ...
  const authoritativeItems: Memory[] = live?.snapshot ? live.snapshot.memory.facts.map((fact, index) => ({ id: fact.name || `memory-${index}`, text: fact.description || fact.body, scope: fact.type === "project" ? (liveProjects[0]?.name ?? "项目") : "全局", type: fact.type === "project" ? "项目" : fact.type === "user" ? "用户" : "偏好" })) : items;
  const scopeOptions = ["全部", "全局", ...liveProjects.map((p) => p.name)];
  const list = (!live && pstate === "empty" ? [] : authoritativeItems).filter((m) =>
  ...
  const save = (m: Memory, currentProject: string) => {
    ...
    if (m.type === "项目" && m.scope !== currentProject && !liveProjects.some((p) => p.name === m.scope)) return toast.error("无效的项目范围");
    if (live) {
      void live.apply("保存记忆", async () => {
        if (!live.backend.Remember) throw new Error("当前桌面后端缺少记忆保存绑定");
        await live.backend.Remember(m.type === "项目" ? "project" : "user", m.text.trim());
      }).then((ok) => { if (ok) { toast.success("记忆已保存"); setEditing(null); } else toast.error("记忆保存失败", { description: live.error }); });
      return;
    }
  ...
  return (
    <SettingsBody title="记忆" desc={`管理${brand.productName}记住的用户偏好与项目事实；凭据不会被保存，项目记忆彼此隔离`}>
      {!live && <StateSwitcher value={pstate} onChange={setPState} options={[{ id: "normal", label: "正常" }, { id: "empty", label: "无记忆" }]} />}
      ...
      {live && <div className="mb-4"><Section title="记忆能力" desc="关闭后不再编译新记忆；现有记忆仍可查看和清理"><Row label="启用记忆编译"><Toggle checked={live.snapshot?.settings.memoryCompilerEnabled ?? false} onChange={(enabled) => { void live.apply(enabled ? "启用记忆" : "停用记忆", async () => { if (!live.backend.SetMemoryCompilerEnabled) throw new Error("当前桌面后端缺少记忆开关绑定"); await live.backend.SetMemoryCompilerEnabled(enabled); }).then((ok) => ok ? toast(enabled ? "已启用记忆" : "已停用记忆") : toast.error("记忆设置保存失败", { description: live.error })); }} /></Row></Section></div>}
      ...
      <Section title="记忆列表" actions={
        ...
        {list.length === 0 ? (
          <div className="grid place-items-center py-10 text-center text-[13px] text-slate-400"><Brain className="size-7 text-slate-300" /><p className="mt-2">{authoritativeItems.length === 0 || (!live && pstate === "empty") ? "还没有任何记忆" : "没有匹配的记忆"}</p></div>
        ) : (
          <div className="space-y-2">
            {list.map((m) => (
              <div key={m.id} className="flex items-start gap-3 rounded-lg border border-slate-200 p-3">
                ...
                {!live && <button onClick={() => setEditing(m)} className="grid size-7 shrink-0 place-items-center rounded-lg text-slate-500 ring-1 ring-slate-200 hover:bg-slate-50"><Pencil className="size-3.5" /></button>}
      ...
      <ConfirmDialog open={!!deleteId} title="删除记忆" confirmText="删除" onConfirm={() => {
        if (live && deleteId) void live.apply("删除记忆", async () => { if (!live.backend.Forget) throw new Error("当前桌面后端缺少记忆清理绑定"); await live.backend.Forget(deleteId); }).then((ok) => { if (ok) { setDeleteId(null); toast.success("已删除该记忆"); } else toast.error("记忆删除失败", { description: live.error }); });
        else { setItems((is) => is.filter((x) => x.id !== deleteId)); setDeleteId(null); toast.success("已删除该记忆"); }
