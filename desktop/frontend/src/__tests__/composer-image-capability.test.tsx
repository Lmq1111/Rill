...

{
  ...
  installBridgeApp({
    SavePastedImage: async () => {
      saveCalls += 1;
      return ".rillagent/attachments/mock.png";
    },
...

{
  ...
  installBridgeApp({
    SavePastedImage: async () => ".rillagent/attachments/mock.png",
    AttachmentDataURL: async () => "data:image/png;base64,iVBORw0KGgo=",
  });
  ...
  ok(sent[0]?.submit?.includes("@.rillagent/attachments/mock.png") === true, "submitted text retains the local image attachment ref");
...

{
  ...
  const { root, paint } = renderUserMessage("check @[photo.png](.rillagent/attachments/mock.png)");
  await paint();
