...
func TestInstallerCommandLineIsSilentAndKeepsDFlagLast(t *testing.T) {
	got := installerCommandLine(`C:\Temp\Rill Installer.exe`, `D:\Tools\Rill App`)
	want := `"C:\Temp\Rill Installer.exe" /S /D=D:\Tools\Rill App`
	if got != want {
		t.Fatalf("installerCommandLine = %q, want %q", got, want)
	}
	if !strings.HasSuffix(got, `/D=D:\Tools\Rill App`) {
...
func TestWindowsUpdateHandoffArgsCarryParentInstallAndRelaunch(t *testing.T) {
	got := windowsUpdateHandoffArgs(
		4242,
		`C:\Users\Jane Doe\AppData\Local\Rill\updates\Rill-windows-amd64-installer.exe`,
		`D:\Tools\Rill App`,
		`D:\Tools\Rill App\rill-desktop.exe`,
		"v1.6.0",
	)
	want := []string{
		"--parent-pid", "4242",
		"--installer", `C:\Users\Jane Doe\AppData\Local\Rill\updates\Rill-windows-amd64-installer.exe`,
		"--to-version", "v1.6.0",
		"--install-dir", `D:\Tools\Rill App`,
		"--relaunch", `D:\Tools\Rill App\rill-desktop.exe`,
	}
...
func TestWindowsInstallerScriptWaitsBeforeCopyingExecutable(t *testing.T) {
	...
	wait := strings.Index(script, "Call rillagent.waitForExecutableUnlock")
