...
func runAssist(args []string) int {
	fs := flag.NewFlagSet("rill-guard assist", flag.ContinueOnError)
	...
	allowProject := fs.Bool("allow-project", false, "allow a plan to quarantine project rillagent.toml")
...
func runApplyPlan(args []string) int {
	fs := flag.NewFlagSet("rill-guard apply-plan", flag.ContinueOnError)
	root := fs.String("root", ".", "project root")
	file := fs.String("file", "", "RepairPlan JSON file")
	yes := fs.Bool("yes", false, "confirm plan application non-interactively")
	allowProject := fs.Bool("allow-project", false, "allow project rillagent.toml repair")
...
const repairPlanSystemPrompt = `You are a Rill recovery planner. Return JSON only, matching exactly:
...
func providerSafeScope(scope string) string {
	...
	case "global", "project", "runtime", "network", "model", "provider", "plugin", "permissions",
		"global config", "credential file", "Rill home", "Rill state directory", "project root":
		return scope
	}
	return "other"
}
