// Command rill-guard diagnoses and repairs Rill without loading the
...
func run(args []string) int {
	...
	case "version", "--version", "-v":
		fmt.Println("rill-guard", version)
		return 0
	case "help", "--help", "-h":
		usage()
		return 0
...
func runDiagnose(args []string) int {
	fs := flag.NewFlagSet("rill-guard diagnose", flag.ContinueOnError)
	...
	if *jsonOut {
		...
		fmt.Println("Rill Guard diagnostics")
...
func runRebuild(args []string) int {
	fs := flag.NewFlagSet("rill-guard rebuild", flag.ContinueOnError)
...
func runSnapshots(args []string) int {
	fs := flag.NewFlagSet("rill-guard snapshots", flag.ContinueOnError)
	jsonOut := fs.Bool("json", false, "print JSON")
	if err := fs.Parse(args); err != nil || fs.NArg() != 0 {
		return 2
	}
...
func runRestore(args []string) int {
	fs := flag.NewFlagSet("rill-guard restore", flag.ContinueOnError)
...
func runUndo(args []string) int {
	fs := flag.NewFlagSet("rill-guard undo", flag.ContinueOnError)
	jsonOut := fs.Bool("json", false, "print JSON")
	if err := fs.Parse(args); err != nil || fs.NArg() != 0 {
		return 2
	}
...
func runCheck(args []string, apply bool) int {
	name := "rill-guard check"
	if apply {
		name = "rill-guard repair"
	}
	fs := flag.NewFlagSet(name, flag.ContinueOnError)
	root := fs.String("root", ".", "project root to inspect")
	includeProject := fs.Bool("project", false, "allow repair to quarantine project rillagent.toml")
	jsonOut := fs.Bool("json", false, "print JSON")
	if err := fs.Parse(args); err != nil || fs.NArg() != 0 {
		return 2
	}
...
func runLaunch(args []string) int {
	fs := flag.NewFlagSet("rill-guard launch", flag.ContinueOnError)
	...
	if result, failure, err := repair.RecoverFailedInstall(); err != nil {
		...
		if failedInstallBlocksLaunch(result, err) {
			fmt.Fprintln(os.Stderr, "error: the failed installer may have left a mix of two releases and the rollback did not complete; refusing to start. Re-run rill-guard to retry the rollback, or reinstall Rill.")
			return 1
		}
	} else if failure != nil && result.RolledBack {
		fmt.Fprintf(os.Stderr, "Rill Guard restored %s after the %s installer failed.\n", result.ToVersion, result.FromVersion)
	}
	...
	if !*safeMode && tracker.SafeModeRecommended() {
		if result, err := repair.RollbackPendingUpdate(); err != nil {
			...
			if result.MixedInstall {
				fmt.Fprintln(os.Stderr, "error: the installation now mixes two releases; refusing to start. Re-run rill-guard to retry the rollback, or reinstall Rill.")
				return 1
			}
			...
			fmt.Fprintf(os.Stderr, "Rill Guard restored %s after %s failed to start.\n", result.ToVersion, result.FromVersion)
			_ = tracker.MarkClean()
		} else {
	...
	if path == "" {
		fmt.Fprintln(os.Stderr, "error: cannot locate rill-desktop; pass --app PATH")
		return 1
	}
...
func packagedDetachedLauncher() bool {
	...
	return packagedDetachedLauncherName(name)
}

func packagedDetachedLauncherName(name string) bool {
	name = strings.ToLower(filepath.Base(strings.TrimSpace(name)))
	return name == "rill-launcher.exe" || name == "rill.exe"
}

func runRecover(args []string) int {
	fs := flag.NewFlagSet("rill-guard recover", flag.ContinueOnError)
	root := fs.String("root", ".", "project root to inspect")
	includeProject := fs.Bool("project", false, "allow repair to quarantine project rillagent.toml")
	app := fs.String("app", "", "desktop executable path")
	if err := fs.Parse(args); err != nil || fs.NArg() != 0 {
...
func siblingDesktopExecutable() string {
	...
	name := desktopExecutableName(runtime.GOOS)
...
func desktopExecutableName(goos string) string {
	if goos == "windows" {
		return "rill-desktop.exe"
	}
	return "rill-desktop"
}
...
func usage() {
	fmt.Println("usage: rill-guard <check|repair|diagnose|rebuild|assist|apply-plan|launch|recover|snapshots|restore|undo> [options]")
}
