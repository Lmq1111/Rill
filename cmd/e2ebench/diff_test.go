...
func TestEnv(t *testing.T) {
	if got := os.Getenv("RILLAGENT_E2E_ENV"); got != "ok" {
		t.Fatalf("RILLAGENT_E2E_ENV = %q", got)
	}
}
`)

	ok, out := runTests(repo, "GOWORK=off RILLAGENT_E2E_ENV=ok go test", []string{"./..."})
	if !ok {
...
func TestRunTestsRejectsDynamicEnvAssignment(t *testing.T) {
	ok, out := runTests(t.TempDir(), "RILLAGENT_E2E_ENV=$(echo ok) go test", []string{"./..."})
	if ok {
