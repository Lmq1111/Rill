...
agent. It is the Rill analog of Claude Code's CLAUDE.md.
...
- Hierarchical docs: `RILL.md` (this file, committed/shared), `RILL.local.md`
  (personal, git-ignored), user-global `~/.rillagent/RILL.md`, and any
  `RILL.md` in an ancestor dir. `AGENTS.md` is accepted as a fallback name.
...
CI also runs the complete root and desktop Go suites, builds `rillagent`, and
runs frontend type-checking, tests, and the production build with pnpm 10.
...
- **Do not force-push shared branches or rewrite public history.** Add focused
  follow-up commits for review feedback and let the repository's merge policy
  decide whether to squash them.
- **Keep the PR diff minimal.** Only the files relevant to the PR's purpose — no
  stray changes from other branches.
