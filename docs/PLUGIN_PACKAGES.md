# Rill Plugin Packages

Rill plugin packages bundle skills, hooks, and MCP servers behind one
installable unit.

## CLI Mode

Use `rillagent plugin` when installing or managing plugin packages from a
terminal. Plugin packages are installed globally under the Rillagent home
directory.
...
- A local directory that contains `rillagent-plugin.json`,
...
- `--link` links a local plugin directory instead of copying it into Rill's
  plugin storage. Moving or deleting that directory breaks the linked plugin.

Running `rillagent plugin install <source>` without `--dry-run` or `--yes`
...
Rill loads its capabilities into normal interactive sessions:
...
- **MCP servers** join the normal MCP/tool flow. Ask for the task you want done;
  Rill can call the plugin's tools when they are relevant.

After installing, enabling, disabling, or updating a plugin from a separate
terminal while a session is already running, start a new `rillagent` session or
...
- **Developer mode: link source folder** appears for **Local folder** installs.
  It links the selected directory instead of copying it into Rill's plugin
...
Rill plugins can declare `rillagent-plugin.json` at the plugin root:
...
Relative paths are resolved inside the plugin root. Rill does not run
third-party install scripts during plugin installation.

## Codex & Claude Compatibility

Rill also reads Codex plugin manifests at `.codex-plugin/plugin.json` and
...
Rill construct; it does not by itself guarantee every runtime decision an
...
such as Superpowers and Claude-style skill packs, Rill maps:

- `skills` to Rill skill roots. A Claude manifest that declares no
...
- `commands/` (and `.claude/commands/`) to Rill custom slash commands: each
  ...
  details report that conflict. Native `rillagent-plugin.json` manifests can
...
- `agents/*.md` to manually invoked, plugin-owned subagent profiles. Claude
  model aliases inherit the active Rill model; inline `tools` lists map to
  Rill tool names, including wildcard MCP names such as `mcp__*__search`.
  Agents use `/<plugin>:agent:<name>`, so an upstream agent and skill may share
  the same name without shadowing one another.
- `hooks/session-start-codex` to the Rill `SessionStart` hook when present.
- A plugin-root `CLAUDE.md` file to a built-in `SessionStart` context hook. The
  file is read directly by Rill, without spawning a shell command.
- `.claude/settings.json` and `hooks/hooks.json` command hooks to Rill hook
  ...
  translated between Rill's own tool names and Claude's (`bash` ↔
  `Bash`, `write_file` ↔ `Write`, ...), so a matcher like `"Bash"` fires
  correctly; every Rill subagent-spawning tool (`task`, `read_only_task`,
  ...
  includes Claude's required `prompt` and `description`; Rill supplies a
  stable operation label when its tool call omitted the optional description.
  `tool_input` keys that
  Rill names differently from Claude are renamed too — `path` becomes
  ...
  task content. `NotebookEdit` also supplies `new_source` from Rill's
  ...
  interrupted}` shape (Rill combines both streams into `stdout`; the
  ...
  expands `${CLAUDE_PLUGIN_ROOT}` and `${RILLAGENT_PLUGIN_ROOT}` (plus their
  ...
  `Stop`/`SubagentStop` hook (which cannot block the turn in Rill), or a
  matcher that covers one of three inputs Rill cannot losslessly express:
  `WebFetch.prompt`, `NotebookEdit.cell_id` for a Rill `cell_number` call,
  or `TaskOutput.task_id` when Rill `wait` covers multiple/all jobs. Each
...
Unsupported Claude hook item types are skipped with a warning. Rill does not
