...
with `runAs: subagent`: Rill starts an isolated child agent, gives it the
...
```bash
rillagent subagent create reviewer \
...
`.rillagent/skills/<name>/SKILL.md`; global profiles are stored under the
Rillagent home Skill directory described in
...
printf '%s\n' 'Review the task and report only actionable findings.' | \
  rillagent subagent create reviewer --description "Code reviewer"
```

Names may contain letters, digits, `_`, `-`, and `.`. Rill refuses a name
...
rillagent subagent try reviewer "review the current diff"
...
git diff | rillagent subagent run reviewer --max-steps 20
```
...
apply. Ordinary `rillagent run` remains a plain one-shot task entry point and
does not implicitly interpret `/<profile>` syntax.

## Manage profiles

```text
rillagent subagent list [--dir PATH]
rillagent subagent create <name> --description TEXT (--prompt TEXT | --prompt-file PATH)
  [--scope project|global] [--model REF] [--effort LEVEL]
  [--tools a,b] [--color NAME] [--dir PATH]
rillagent subagent edit <name> [--description TEXT]
  [--prompt TEXT | --prompt-file PATH] [--model REF] [--effort LEVEL]
  [--tools a,b] [--color NAME] [--dir PATH]
rillagent subagent delete <name> --yes [--dir PATH]
rillagent subagent try <name> [--model REF] [--max-steps N] [--dir PATH] <task>
rillagent subagent run <name> [--model REF] [--max-steps N] [--dir PATH] <task>
```
...
```bash
rillagent subagent edit reviewer --model= --effort= --tools= --color=
```
...
Profiles created in desktop settings and with `rillagent subagent create` share
...
`rillagent subagent list`, the current `--dir`, and `skills.disabled_skills`. If
...
invalid effort levels are rejected when Rill resolves the effective model.
