...
> 如何在 IM 里使用 Rill，以及审批、问答、YOLO 和常用命令的交互方式。
...
连接机器人后，你可以在飞书、Lark、微信或 QQ 里给 Rill 发消息，让桌面端
Rill 或 `rillagent bot start` 进程在本机执行同一套模型、工具、权限与
沙盒逻辑。

典型场景：

- 让 Rill 查代码、读文档、解释错误、整理结论。
...
- **桌面端 runtime**：在 **设置 -> 机器人** 中配置。桌面端会启动 gateway，
  ...
  本地 IM 会话。
- **CLI runtime**：执行 `rillagent bot start` 启动无界面长期进程。它复用
  与桌面端相同的配置、白名单、路由、队列设置、配对存储、适配器和
  项目/会话索引。

普通 `rillagent run` 不会自动启动 IM 网关。只有桌面端 bot runtime 正在运行，
或存在一个存活的 `rillagent bot start` 进程时，远端 IM bot 能力才会生效。

## 连接四个渠道

打开桌面端 Rill，进入 **设置 -> 机器人**。在 **添加 IM Bot** 区域选择
渠道并扫码。
...
一次性配对码。配对码需要在本机执行 `rillagent bot pairing approve <code>`
...
命令能力。远端用户进入的是同一个 Rill controller、权限策略、工具
...
sequenceDiagram
  ...
  participant R as "Rill 桌面端或 bot start"
...
| `/projects [关键词]` | 查看已索引项目工作区 | `/projects rillagent` |
...
可以在配置里限制排队堆积；`rillagent bot doctor --deep` 会显示当前队列、
配对和角色诊断信息。
...
`.rillagent/attachments`，并以 `@.rillagent/attachments/...` 形式传给
Rill。保存失败的附件会在 IM 中提示，文本内容仍会继续处理。内置
Feishu、Weixin、QQ 适配器当前仍以文本事件为主，普通 IM 附件抽取可以继续在
适配器层补齐。

## 审批与 YOLO

Rill 的机器人沿用桌面端权限系统。默认是询问模式：写文件、执行命令等
敏感工具调用会先请求确认。
...
正常升级或覆盖安装 Rill app 后，不需要重新绑定。

绑定信息保存在用户配置目录，而不是 app 包内：

- Bot 连接、远端 ID、白名单、模型和审批模式保存在用户配置文件。
- 飞书和 Lark 的密钥保存在 CLI 与桌面端共用的 Rill 全局
  `<Rillagent home>/.env`。
- 微信扫码后的账号 token 保存在 Rill 的用户数据目录。
- QQ 的 App ID 保存在用户配置文件；App Secret 通过配置的环境变量
  （默认 `QQ_BOT_APP_SECRET`）保存在 Rill 全局凭据文件中。
...
| 已连接但没有回复 | 确认桌面端 bot runtime 或 `rillagent bot start` 进程正在运行，Bot 连接已开启，用户 ID 在白名单内、已配对或允许所有人。 |
