...
Rill already has Goal mode and AutoResearch instructions. When a goal looks
long-running, `activeGoalBlock` injects an AutoResearch protocol that asks the
model to create `.rillagent/autoresearch/<task-id>/` and maintain files such as
...
- No network, publish, payment, credential, or destructive-operation bypass.
  Existing Rill gates still apply.
...
```text
.rillagent/autoresearch/<task-id>/
  state/
    task_spec.json
    progress.json
...
2. If AutoResearch is on, the controller creates a task unless the goal contains
   an explicit `.rillagent/autoresearch/<task-id>/` path.
...
- Explicit: a goal or prompt includes `.rillagent/autoresearch/<task-id>/`.
...
`AutoResearchOpenTask` opens `.rillagent/autoresearch/<task-id>/` in the
...
- Resume: starts or continues `/goal --research .rillagent/autoresearch/<task-id>/`
...
- explicit `.rillagent/autoresearch/<task-id>/` resumes existing state
...
No existing `.rillagent/autoresearch` task should be deleted or rewritten without
