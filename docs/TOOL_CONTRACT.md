...
This document records the provider-visible contract for Rill compile-time built-in tools. It is generated from the same canonical schema path used by the runtime registry.
...
In a default full-token boot, Rill sends the built-in tools above plus the
...
In token economy mode, Rill starts with nine tools: four direct coding tools,
