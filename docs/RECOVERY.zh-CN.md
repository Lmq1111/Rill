...
Rill 随桌面端提供一个很小的恢复程序 `rill-guard`。它不加载 Wails、
...
`rill-guard` 会启动同目录的桌面程序；只读检查请显式使用 `check`。
Windows 安装包的快捷方式使用同一套 Guard 代码编译出的无终端窗口启动器，同时保留
`rill-guard.exe` 供命令行诊断使用。
Windows/Linux 快捷方式启动桌面后会退出启动器；终端中显式执行
`rill-guard launch` 默认等待桌面退出，只有传入 `--detach` 才分离。

除非使用 `repair` 或 `--apply`，否则检查过程只读。执行修复时，无法解析的 TOML
会重命名为带时间戳的 `.rillagent-quarantine-*` 文件；全局配置损坏时，还会尝试恢复
桌面端成功启动后保存的最近健康快照。守卫不会删除凭据 `.env`、会话 JSONL 或项目
源码。只有显式传入 `--project` 时，项目的 `rillagent.toml` 才会被隔离。
...
`rebuild` 不删除派生数据，而是先隔离指定文件，再让 Rill 自动重建。

## 自动安全模式

桌面端会在 Rill 状态目录记录 `starting`、`ready`、`healthy` 和
...
自动更新前，Rill 会完整保留安装的发布单元——桌面可执行文件以及安装器同样会
...
Rill 修复状态目录；任何任意目标路径、目录外备份或缺失哈希的备份都会被拒绝。
...
所有新增状态文件都是可选且向后兼容的；旧版 Rill 会直接忽略，缺失新字段时按安全
零值处理。
