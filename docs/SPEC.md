# Rill Engineering Spec

> Rill is a coding agent: a thin harness driving multiple models, with **all
...
├── rillagent.example.toml        # sample config
├── docs/SPEC.md             # this file
├── cmd/rillagent/main.go          # entry; blank-imports built-in providers/tools
├── cmd/rillagent-plugin-example/ # reference MCP stdio plugin (a runnable example)
└── internal/
    ...
    ├── command/             # custom slash commands loaded from .rillagent/commands/*.md
...
- `cmd/rillagent-plugin-example` is a runnable reference stdio server (`echo`,
...
Long tasks eventually fill the model's context window. Rill manages this with
...
- The dropped originals are archived under the user config dir
  (`rillagent/archive/<timestamp>.jsonl`; see §5 for its per-OS location), one
...
- **Resolving `Ask`.** The interactive front-end (the chat TUI) prompts the user
  ...
  non-interactive run
  (`rillagent run`, a sub-agent, anything with no TTY / no approver) cannot prompt, so
...
- **Collaboration mode is separate from tool approval.** The desktop composer
  ...
  `.rillagent/autoresearch/YYYYMMDD-HHMMSS-slug/` and keeps dynamic run state out
  of `RILL.md`, `AGENTS.md`, project memory, tool schemas, and the
...
Out of the box (`mode = "ask"`, no rules) `rillagent run` behaves exactly as before
(writers resolve `Ask`→allow with no TTY), while `rillagent` now prompts before
...
- **Custom commands** are Markdown files under `.rillagent/commands/` (project) and
  the user config dir, e.g. `~/.rillagent/commands/` on macOS/Linux; the project dir overrides the user dir on a
...
- Frontmatter is an optional `---`-fenced block of simple `key: value` lines;
  `description` and `argument-hint` are recognised (no YAML dependency — Rill
...
- `rillagent subagent try <name> ... <task>` uses the read-only Skill runner;
- `rillagent subagent run <name> ... <task>` uses the normal permission and
  sandbox path; and
- ordinary `Controller.Run` / `rillagent run` remains unchanged and does not
...
Resolution order: **flag > project `./rillagent.toml` > the user config file
...
Fields marked user/global only are not overridden by project `rillagent.toml`.
...
credential files, and the OS keyring are not provider-key runtime fallbacks. Project `.env` still
feeds workspace-scoped, non-provider `${VAR}` expansion for MCP/plugin settings
without importing provider keys or Rill control variables. Project
`rillagent.toml` does not override the user-level Memory v5 compiler switch.

```toml
default_model = "deepseek"   # provider name (→ its default model) or "provider/model"
# language    = "zh"                # ui language tag; empty = auto-detect from $LANG / $RILLAGENT_LANG
...
[agent]
system_prompt = "You are Rill, a coding agent..."  # or system_prompt_file = "..."
temperature       = 0.0
memory_compiler = { enabled = true, verbosity = "observe" }   # user/global only; observe|compact; CLI: rillagent config memory-v5 off|observe|compact|on|status
...
parseable for upgrade compatibility, but are ignored and removed when the
configuration is normalized. The CLI `--max-steps` flag and `[bot].max_steps` remain separate,
explicit controls for one-off and unattended execution.

`rillagent setup` writes this default config so the CLI is usable out of the box.
...
`[serve]` controls the HTTP browser frontend used by `rillagent serve`. The
...
`[[plugins]]`; on a name collision `rillagent.toml` wins (it is the more explicit,
Rill-specific source). This lets a server already configured for Claude work in
Rill unchanged.
...
`multi_edit`, `move_file`) to `workspace_root` (default cwd), the Rill user
...
`network = true`.
**Windows status:** Rill does not ship an OS-level Bash sandbox on Windows.
The effective mode is fixed to `off`; an older config containing
`bash = "enforce"` remains readable but resolves to `off`, `rillagent doctor`
...
- Build: `CGO_ENABLED=0 go build -ldflags "-s -w -X main.version=$(VERSION)" -o rillagent ./cmd/rillagent`
...
- Sandbox Phase 1: an OS-level jail for `bash` so commands — not just the
  ...
  offer an explicit, permission-gated unconfined rerun (in `rillagent run`, the
...
- "Always allow" persistence writing learned rules back to project config; a
  per-session permission override flag for `rillagent run`.
