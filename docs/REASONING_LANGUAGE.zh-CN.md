...
`agent.reasoning_language` 控制模型服务暴露“可见思考过程”时，Rill 希望它优先使用哪种语言。
...
桌面端设置会写入用户级默认值。项目仍然可以通过 `./rillagent.toml` 覆盖。
...
```bash
rillagent config reasoning-language auto
rillagent config reasoning-language zh
rillagent config reasoning-language en
```

默认写入用户配置。要写入当前项目的覆盖配置：

```bash
rillagent config reasoning-language --local zh
```

在 `rillagent` 内可以用斜杠命令：
...
斜杠命令会写入用户级设置，并立即更新当前 chat controller，后续 turn 生效。它不会改写当前项目的 `rillagent.toml`；如果要写项目级覆盖，请使用带 `--local` 的 shell 命令。

单次 headless 运行也会读取同一设置：

```bash
rillagent run "解释这个模块"
```
...
```text
./rillagent.toml > 用户 config.toml > 内置默认值
```
...
`auto` 仍然对缓存友好。用户原始提问明显是中文时，Rill 会为这一轮加入同样很小的 `<reasoning-language>` 临时 block；英文或信号不明确时不注入，只复用已有的稳定语言策略。`@file` 等注入的引用内容不会参与这个自动判断。

当设置为 `zh` 或 `en` 时，Rill 总是会把一个很小的 `<reasoning-language>` 临时 block 放进本次 user turn。所有模式下，它都不会改变：
