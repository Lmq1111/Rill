...
Thank you for your interest in contributing to Rill! This guide covers
...
cd Rill
go build ./cmd/rillagent    # builds the CLI binary
...
| `internal/memory` | RILL.md hierarchy + auto-memory |
...
| `docs/` | Engineering spec, guides, and data-path contract |
...
with `RILLAGENT_HOME` set. This gives each build its own self-contained directory
...
```bash
RILLAGENT_HOME=/tmp/rillagent-dev go run ./cmd/rillagent
# or after building:
#   RILLAGENT_HOME=/tmp/rillagent-dev ./bin/rillagent
```
...
RILLAGENT_HOME=/tmp/rillagent-dev-isolated build/bin/rill-desktop
```

On Windows, use `$env:RILLAGENT_HOME` in PowerShell or `set RILLAGENT_HOME=` in
...
logs — stays under `RILLAGENT_HOME`. Rill does not scan or import another
product's configuration or state, so no production data leaks in or out.

### Cache-first review gate

Rill treats high prompt-cache hit rate as product behavior. Changes that
...
Open an issue in [Lmq1111/Rill](https://github.com/Lmq1111/Rill/issues) with:
- Steps to reproduce
