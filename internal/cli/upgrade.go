...
// upgradeCommand handles `rillagent upgrade` (and `rillagent update`).
func upgradeCommand(args []string, version string) int {
	...
	base := fmt.Sprintf("rillagent-%s-%s", runtime.GOOS, runtime.GOARCH)
	var asset *ghAsset
...
// "npm-v"). Prereleases are kept: only 1.x carries `rillagent upgrade`, and the
...
func fetchLatestRelease(c *http.Client) (*ghRelease, error) {
	...
	req.Header.Set("User-Agent", "rillagent-cli")
...
// extractBinary pulls the "rillagent" binary from a .tar.gz or .zip archive.
...
// rename it aside to .rillagent.old first, then place the new binary.
