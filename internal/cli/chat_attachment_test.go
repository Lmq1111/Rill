...
func TestExpandPastedBlocksImage(t *testing.T) {
	m := &chatTUI{pastedBlocks: []pastedBlock{
		{label: "[image #1]", text: "@.rillagent/attachments/clipboard-20260601-010203.000001.png", image: true},
		{label: "[Pasted text #2 · 3 lines]", text: "a\nb\nc"},
	}}
	got := m.expandPastedBlocks("look at [image #1] and [Pasted text #2 · 3 lines]")
	want := "look at @.rillagent/attachments/clipboard-20260601-010203.000001.png and " +
...
func TestDisplayLineForImageRefs(t *testing.T) {
	got := displayLineForImageRefs("describe @.rillagent/attachments/clipboard-20260601-010203.000001.png @.rillagent/attachments/clipboard-20260601-010204.000002-000002.jpg")
...
func TestPasteShellEscapedImagePathInsertsImageToken(t *testing.T) {
	...
	if text := updated.pastedBlocks[0].text; !strings.HasPrefix(text, "@.rillagent/attachments/clipboard-") || !strings.HasSuffix(text, ".png") {
...
func TestMissingPastedImagePathRemainsText(t *testing.T) {
	content := `/definitely-missing/rillagent-image.png`
	if runtime.GOOS == "windows" {
		content = `C:/definitely-missing/rillagent-image.png`
	}
