...
func doctorRepairCommand(args []string) int {
	...
	includeProject := fs.Bool("project", false, "allow --apply to quarantine an invalid project rillagent.toml")
	jsonOut := fs.Bool("json", false, "print result as JSON")
	if err := fs.Parse(args); err != nil {
		return 2
	}
	if fs.NArg() != 0 {
		fmt.Fprintln(os.Stderr, "usage: rillagent doctor repair [--root PATH] [--apply] [--project] [--json]")
		return 2
	}
	...
	if *jsonOut {
		...
		fmt.Println("Rill repair report")
...
func doctorQualityCommand(args []string, version string) int {
	...
	for _, arg := range args {
		switch arg {
		case "-h", "--help":
			fmt.Fprintln(os.Stdout, "usage: rillagent doctor quality <branch-id-or-path> [--json]")
		...
		default:
			if strings.HasPrefix(arg, "-") || ref != "" {
				fmt.Fprintln(os.Stderr, "usage: rillagent doctor quality <branch-id-or-path> [--json]")
				return 2
			}
			ref = arg
		}
...
func doctorRedactSessionsCommand(args []string) int {
	...
	if fs.NArg() != 0 {
		fmt.Fprintln(os.Stderr, "usage: rillagent doctor redact-sessions [--dry-run] [--json] [--dir PATH]")
		return 2
	}
...
func doctorSessionCommand(args []string, version string) int {
	...
	for i := 0; i < len(args); i++ {
		...
		switch arg {
		case "-h", "--help":
			fmt.Fprintln(os.Stdout, "usage: rillagent doctor session <branch-id-or-path> [--zip] [--out PATH]")
			fmt.Fprintln(os.Stdout, "")
			fmt.Fprintln(os.Stdout, "Bundles the session transcript, persistence sidecars, conflict diagnostics,")
			fmt.Fprintln(os.Stdout, "and the recovery parent chain into a zip for support. Unlike `rillagent doctor`,")
		...
		default:
			...
			if ref != "" {
				fmt.Fprintln(os.Stderr, "usage: rillagent doctor session <branch-id-or-path> [--zip] [--out PATH]")
				return 2
			}
			ref = arg
		}
