...
const resumePickerSentinel = "__rillagent_resume_picker__"
...
// the top-level flag run, i.e. before any "--" terminator. rillagent has no
// interactive -p, so its presence means the user wants one-shot print mode even
// when it trails other flags (`rillagent --model X -p "task"`).
