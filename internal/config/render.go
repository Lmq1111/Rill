...
func RenderTOMLForScope(c *Config, scope RenderScope) string {
	...
	if scope != RenderScopeProject {
		...
		fmt.Fprintf(&b, "check_updates = %v   # legacy field: Rill always disables background update checks\n", c.DesktopCheckUpdates())
		fmt.Fprintf(&b, "telemetry = %v   # legacy field: Rill never sends upstream telemetry\n", c.DesktopTelemetry())
		fmt.Fprintf(&b, "metrics = %v   # legacy field: Rill never sends upstream metrics\n", c.DesktopMetrics())
