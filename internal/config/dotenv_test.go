...
// TestLoadDotEnvReadsGlobalCredentials proves `rillagent setup`'s target — the
// rillagent-owned credentials file under Rill home — is loaded from any
...
func TestDotEnvFileFilteredPreservesProjectScopeRules(t *testing.T) {
	...
	if _, ok := got["RILLAGENT_HOME"]; ok {
		t.Fatalf("RILLAGENT_HOME should be filtered: %+v", got)
	}
	if _, ok := got["HOME"]; ok {
...
func TestLoadForRootFiltersProjectDotEnvControlVars(t *testing.T) {
	...
	os.Unsetenv("RILLAGENT_STATE_HOME")

	wantCred := UserCredentialsPath()
	if wantCred == "" {
		t.Skip("user credentials path unavailable")
	}
	if err := os.WriteFile(filepath.Join(project, ".env"), []byte("RILLAGENT_STATE_HOME="+redirect+"\nPLUGIN_TOKEN=project-token\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(project, ".mcp.json"), []byte(`{
  "mcpServers": {
    "svc": {
      ...
      "headers": {
        ...
        "State": "${RILLAGENT_STATE_HOME:-default-state}"
      }
    }
  }
}`), 0o644); err != nil {
	...
	if got := os.Getenv("RILLAGENT_STATE_HOME"); got != "" {
...
func TestCredentialResolverCachesGlobalFirstLookups(t *testing.T) {
	...
	stubStoredCredentialValueForTest(t, func(got string) (string, CredentialSource, bool) {
		...
		return "from_credentials", CredentialSource{Kind: CredentialSourceCredentials, Label: "Rill credentials"}, true
	})
...
func TestResolveCredentialSourceShowsCredentialsShadowingProjectEnv(t *testing.T) {
	...
	if !got.Set || got.Source.Kind != CredentialSourceCredentials {
		t.Fatalf("source = %+v set=%v, want Rill credentials", got.Source, got.Set)
	}
...
func TestUserCredentialsPathIgnoresRillStateHome(t *testing.T) {
	home := t.TempDir()
	state := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	t.Setenv("AppData", filepath.Join(home, "AppData"))
	t.Setenv("RILLAGENT_HOME", filepath.Join(home, "rillagent-home"))
	t.Setenv("RILLAGENT_STATE_HOME", state)

	want := filepath.Join(home, "rillagent-home", ".env")
	...
	if strings.HasPrefix(UserCredentialsPath(), state) {
		t.Fatalf("credentials path must not live under RILLAGENT_STATE_HOME: %q", UserCredentialsPath())
	}
}
...
func TestProjectConfigCannotOverrideCredentialStoreMode(t *testing.T) {
	...
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	t.Setenv("RILLAGENT_CREDENTIALS_STORE", "")
	os.Unsetenv("RILLAGENT_CREDENTIALS_STORE")
	...
	if err := os.WriteFile(filepath.Join(project, "rillagent.toml"), []byte(`credentials_store = "keyring"`), 0o644); err != nil {
		t.Fatal(err)
	}
...
// TestLoadDotEnvGlobalCredentialsOverrideEnv confirms Rill-owned global
