...
// loadDotEnv loads Rill's global .env for provider credentials. The
...
// expansion, then loads Rill's global .env for provider credentials.
...
// other and project files cannot redirect Rill's own config/credential
// paths.
...
func isProjectDotEnvControlKey(key string) bool {
	...
	if strings.HasPrefix(upper, "RILLAGENT_") {
		return true
	}
	switch upper {
