...
const systemPrompt = "You are rillagent, a coding agent. Be concise and follow project conventions. " +
...
func TestReleaseCacheHitGuard(t *testing.T) {
	if os.Getenv("RILLAGENT_RELEASE_CACHE_GUARD") == "" {
		t.Skip("set RILLAGENT_RELEASE_CACHE_GUARD=1 to run the release cache guard")
	}

	threshold := envInt("RILLAGENT_CACHE_GUARD_THRESHOLD", 90)
	maxLowCases := envInt("RILLAGENT_CACHE_GUARD_MAX_LOW_CASES", 1)
	...
	if len(lows) > maxLowCases {
		...
		if os.Getenv("RILLAGENT_CACHE_GUARD_STRICT") != "" {
			t.Fatal(msg)
		}
	}
}
