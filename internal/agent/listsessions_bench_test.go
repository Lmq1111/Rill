...
// directory. It is gated behind RILLAGENT_BENCH=1 so it never runs (or writes
// hundreds of MB) during a normal `go test`.
//
//	RILLAGENT_BENCH=1 go test ./internal/agent -run TestListSessionsScalingManual -v
func TestListSessionsScalingManual(t *testing.T) {
	if os.Getenv("RILLAGENT_BENCH") != "1" {
		t.Skip("set RILLAGENT_BENCH=1 to run the scaling benchmark")
	}
...
//
//	RILLAGENT_BENCH=1 go test ./internal/agent -run TestColdStartProjectTreeManual -v
func TestColdStartProjectTreeManual(t *testing.T) {
	if os.Getenv("RILLAGENT_BENCH") != "1" {
		t.Skip("set RILLAGENT_BENCH=1 to run the cold-start benchmark")
	}
