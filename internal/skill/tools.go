...
func (t *installSkillTool) Description() string {
	scope := "'global' (only option — no project workspace) writes to the Rill home skills directory."
	if t.store.HasProjectScope() {
		scope = "'project' (default) writes to <repo>/.rillagent/skills/ (this workspace only); 'global' writes to the Rill home skills directory (every project)."
	}
