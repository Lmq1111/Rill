---
name: rillagent-guide
description: "Troubleshoot and configure Rill capabilities: Skills (project/custom/global/builtin priority, discovery dirs), Commands (override order, /dir:file naming), Hooks (11 events, project trust, matchers, timeouts), MCP (rillagent.toml + .mcp.json + plugin packages, auto_start), plugin packages (native/Codex/Claude manifests), and AGENTS.md / instruction docs. Use when the user asks how to configure, debug missing skills/commands/hooks/MCP/plugins, or diagnose capability loading."
runAs: inline
---

# Rill self-diagnostics guide
...
```bash
rillagent doctor capabilities --live --timeout 5s --json
```
...
1. **project** — `<workspace>/{.rillagent,.agents,.agent,.claude}/skills/`
2. **custom** — `[skills].paths` (and plugin package skill roots)
3. **global** — `<Rill home>/skills` and home convention dirs
...
Discovery conventions: `.rillagent`, `.agents`, `.agent`, `.claude` (see `config.ConventionDirs`). Layouts: `<name>/SKILL.md` or flat `<name>.md` (Claude flat files need skill frontmatter).

### Checks

| Entry | How |
| --- | --- |
| CLI | `rillagent doctor capabilities` → Skills section |
| Desktop | Settings → Skills; Settings → Diagnostics |
| Agent | `/skill` list, `/rillagent-guide`, `run_skill` |
...
1. `rillagent doctor capabilities --json` → Skills
...
`config.CommandDirsForRoot`: home convention commands → Rill home commands → project convention commands. **Later directory overrides earlier** on name clash (`command.Load`).
...
- Project: `<workspace>/.rillagent/settings.json` — **only when trusted** (`trust.json` under Rill home)
- Plugin packages: installed enabled packages
- Global: `<Rill home>/settings.json` (always)
...
- Native: `rillagent-plugin.json`
- Codex: `.codex-plugin/plugin.json`
- Claude: `.claude-plugin/plugin.json` (+ limited Claude compatibility paths)

State: `<Rill home>/plugin-packages.json`. Disabled packages do not contribute skills/hooks/MCP.

Unmapped Claude-only features may appear as compatibility warnings — Rill does not invent support.

### Checks

`rillagent plugin doctor <name>`, Settings → Plugins, Diagnostics → Plugins.
...
## Instructions (AGENTS.md / RILL.md)

### Load order (ascending specificity)

User global docs → ancestor chain → project docs → project-local (`*.local.md`).

Recognized names: `RILL.md`, `AGENTS.md`, `CLAUDE.md` (and `*.local.md` variants). Multiple files in one directory can load; symlink identity is deduped.
