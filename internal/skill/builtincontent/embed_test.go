...
func TestLoadRillagentGuide(t *testing.T) {
	sk, err := builtincontent.LoadRillagentGuide()
	if err != nil {
		t.Fatal(err)
	}
	if sk.Name != "rillagent-guide" {
	...
	if !strings.Contains(sk.Body, "rillagent doctor capabilities") {
