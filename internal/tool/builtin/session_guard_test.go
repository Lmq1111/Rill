...
// stateRootFor builds a fake Rill state root with the two guarded session
...
func TestSessionDataGuardDeniesSessionStores(t *testing.T) {
	...
	for _, target := range []string{
		...
		} else if !strings.Contains(err.Error(), "Rill's own session/state data") {
...
func TestSessionDataGuardZeroValueUnconfined(t *testing.T) {
	...
	if hint := g.CommandHint("", "rm -rf ~/.rillagent/sessions"); hint != "" {
...
func TestSessionDataGuardCommandHintEnvVarForm(t *testing.T) {
	...
	state := filepath.Join(home, ".rillagent")
	...
	for _, cmd := range []string{
		`python3 -c "open('$HOME/.rillagent/sessions/x.jsonl','w')"`,
		"rm ${HOME}/.rillagent/projects/slug/sessions/y.jsonl",
	} {
		if hint := g.CommandHint("", cmd); hint == "" {
...
func TestBashAppendsSessionDataHint(t *testing.T) {
	...
	if !strings.Contains(out, "WARNING: this command referenced Rill's own session/state data") {
