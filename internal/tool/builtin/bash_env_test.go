...
func TestBashMergesLoginShellPath(t *testing.T) {
	...
	probe := filepath.Join(bin, "rillagent-path-probe")
	...
	args, _ := json.Marshal(map[string]string{"command": "rillagent-path-probe"})
...
func TestBashCommandEnvFiltersSensitiveKeysWhenEnabled(t *testing.T) {
	...
	t.Setenv("RILLAGENT_TEST_VISIBLE", "ok")
	...
	if !strings.Contains(env, "RILLAGENT_TEST_VISIBLE=ok") {
...
func TestParseShellPATH(t *testing.T) {
	const marker = "__RILLAGENT_BASH_PATH__="
	cases := []struct {
		name string
		out  string
		want string
