...
func TestEditFile(t *testing.T) {
	...
	out := runTool(t, editFile{}, map[string]any{"path": f, "old_string": "world", "new_string": "rillagent"})
	for _, want := range []string{"Actual replacement receipt after write:", "-world", "+rillagent"} {
		if !strings.Contains(out, want) {
	...
	if b, _ := os.ReadFile(f); string(b) != "hello rillagent\n" {
...
func TestMultiEdit(t *testing.T) {
	...
	out := runTool(t, multiEdit{}, map[string]any{
		...
		"edits": []map[string]any{
			...
			{"old_string": "old", "new_string": "fresh", "replace_all": true},
		},
	})
	if !strings.Contains(out, "multi_edit") || !strings.Contains(out, "2 edits applied") {
		t.Errorf("summary unexpected: %q", out)
	}
	for _, want := range []string{"Actual replacement receipt after write:", "-package old", "+package new", "-old", "+fresh"} {
		if !strings.Contains(out, want) {
			t.Fatalf("multi_edit result should contain %q in actual post-write receipt:\n%s", want, out)
		}
	}
	if strings.Contains(out, "func fresh") {
