...
func TestWorkspaceLeaseHelperProcess(t *testing.T) {
	if os.Getenv("RILLAGENT_WORKSPACE_LEASE_HELPER") != "1" {
		return
	}
	root := os.Getenv("RILLAGENT_WORKSPACE_LEASE_ROOT")
	locks := os.Getenv("RILLAGENT_WORKSPACE_LEASE_DIR")
	ready := os.Getenv("RILLAGENT_WORKSPACE_LEASE_READY")
...
func TestCrossProcessLeaseBlocksAndCrashReleases(t *testing.T) {
	...
	cmd.Env = append(os.Environ(),
		"RILLAGENT_WORKSPACE_LEASE_HELPER=1",
		"RILLAGENT_WORKSPACE_LEASE_ROOT="+root,
		"RILLAGENT_WORKSPACE_LEASE_DIR="+locks,
		"RILLAGENT_WORKSPACE_LEASE_READY="+ready,
	)
