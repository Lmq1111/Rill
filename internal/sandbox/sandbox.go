...
func UnavailableRemediation() string {
	...
	case "windows":
		return "Windows does not currently provide a Rill OS-level Bash sandbox; the effective setting is fixed to \"off\" and shell commands run unconfined."
	default:
...
func BackendUnavailableReason() string {
	...
	default:
		return "this platform has no supported Rill sandbox backend"
	}
}
