...
// Delivery sessions. Worktrees live under Rill-managed state, never inside
...
func Create(ctx context.Context, workspaceRoot, managedRoot string) (Result, error) {
	...
	if managedRoot == "" {
		return Result{}, errors.New("Rill worktree storage is unavailable")
	}
	if err := os.MkdirAll(managedRoot, 0o700); err != nil {
		return Result{}, fmt.Errorf("create Rill worktree storage: %w", err)
	}
	...
	for attempt := 0; attempt < 5; attempt++ {
		...
		branch := fmt.Sprintf("rill/delivery-%s-%s", time.Now().Format("20060102-150405"), id)
...
// IsManagedPath reports whether path belongs to Rill's durable worktree
