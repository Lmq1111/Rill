...
func initRepo(t *testing.T) string {
	...
	git := func(args ...string) string {
		...
		cmd.Env = append(os.Environ(),
			...
			"GIT_AUTHOR_NAME=Rill Test", "GIT_AUTHOR_EMAIL=rillagent@example.invalid",
			"GIT_COMMITTER_NAME=Rill Test", "GIT_COMMITTER_EMAIL=rillagent@example.invalid")
...
func TestCreateManagedWorktreeFromRepositoryFolder(t *testing.T) {
	...
	if !strings.HasPrefix(result.Branch, "rill/delivery-") {
	...
	if _, err := os.Stat(filepath.Join(result.WorktreeRoot, ".rillagent-source")); !os.IsNotExist(err) {
		t.Fatalf("worktree must not contain a Rill marker, stat err = %v", err)
	}
...
func TestCreatePreservesSelectedRepositorySubdirectory(t *testing.T) {
	...
	cmd = exec.Command("git", "-C", repo, "-c", "user.name=Rill Test", "-c", "user.email=rillagent@example.invalid", "commit", "-m", "subdir")
	if out, err := cmd.CombinedOutput(); err != nil {
...
func TestInspectRejectsUncommittedSelectedSubdirectoryWithoutGitMutation(t *testing.T) {
	...
	before, _, err := runGit(context.Background(), repo, "for-each-ref", "--format=%(refname)", "refs/heads/rill/delivery-")
	if err != nil {
		t.Fatal(err)
	}
	...
	after, _, err := runGit(context.Background(), repo, "for-each-ref", "--format=%(refname)", "refs/heads/rill/delivery-")
	if err != nil {
		t.Fatal(err)
	}
	if after != before {
...
func TestCreateSupportsPathsWithSpaces(t *testing.T) {
	...
	git := func(args ...string) {
		cmd := exec.Command("git", append([]string{"-C", repo}, args...)...)
		cmd.Env = append(os.Environ(), "GIT_AUTHOR_NAME=Rill Test", "GIT_AUTHOR_EMAIL=rillagent@example.invalid", "GIT_COMMITTER_NAME=Rill Test", "GIT_COMMITTER_EMAIL=rillagent@example.invalid")
		if out, err := cmd.CombinedOutput(); err != nil {
