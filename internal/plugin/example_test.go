...
// buildExamplePlugin compiles cmd/rillagent-plugin-example into a temp binary and
...
func buildExamplePlugin(t *testing.T) string {
	t.Helper()
	bin := filepath.Join(t.TempDir(), "rillagent-plugin-example")
	if runtime.GOOS == "windows" {
		bin += ".exe"
	}
	out, err := exec.Command("go", "build", "-o", bin, "reasonix/cmd/rillagent-plugin-example").CombinedOutput()
	if err != nil {
