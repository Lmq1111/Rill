...
func TestResearchGoalCreatesHostManagedAutoResearchTask(t *testing.T) {
	...
	for _, rel := range []string{
		...
		path := filepath.Join(root, ".rillagent", "autoresearch", state.AutoResearchTaskID, rel)
	...
	readJSONFileForTest(t, filepath.Join(root, ".rillagent", "autoresearch", state.AutoResearchTaskID, "state", "task_spec.json"), &spec)
...
func TestResearchGoalRepeatedSetReusesAutoResearchTask(t *testing.T) {
	...
	entries, err := os.ReadDir(filepath.Join(root, ".rillagent", "autoresearch"))
	if err != nil {
...
func TestResearchGoalMissingExplicitTaskBlocksInsteadOfCreatingNewTask(t *testing.T) {
	...
	c.SetGoalWithResearchMode("resume .rillagent/autoresearch/missing-task/", GoalResearchOn)
