...
func TestIsReadOnlyBashSubject(t *testing.T) {
	tests := []struct {
		...
		{"awk 'BEGIN{system(\"touch /tmp/rillagent-awk-test\")}'", false},
		{"awk 'BEGIN{print \"evil\" > \"/tmp/rillagent-awk-test\"}'", false},
		{"sed 's/a/b/' file.txt", false},
		{"sed -e 's/.*/touch \\/tmp\\/rillagent-sed-test/e' file.txt", false},
