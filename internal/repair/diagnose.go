...
func Diagnose(ctx context.Context, opts DiagnoseOptions) (DiagnosticReport, error) {
	...
	for _, check := range configReport.Checks {
		if check.Exists && !check.Valid {
			valid = false
			report.add("error", "config.invalid_toml", check.Scope, "Configuration cannot be parsed: "+check.Error, "Run rill-guard repair; add --project only for a project config.")
		}
	}
	...
	checkDirectoryMode(&report, config.RillHomeDir(), "Rill home")
	if config.MemoryUserDir() != config.RillHomeDir() {
		checkDirectoryMode(&report, config.MemoryUserDir(), "Rill state directory")
	}
...
func validateProviders(report *DiagnosticReport, cfg *config.Config) {
	...
	for i := range cfg.Providers {
		...
		if entry.Kind != "openai" && entry.Kind != "anthropic" {
			report.add("error", "provider.unsupported_kind", scope, fmt.Sprintf("Provider kind %q is not registered by packaged Rill builds.", entry.Kind), "Use openai or anthropic compatibility.")
		}
		...
		if entry.APIKeyEnv != "" && !config.IsValidCredentialKey(entry.APIKeyEnv) {
			...
			report.add("warning", "provider.missing_key", scope, "The configured API key is missing from the global Rill credential file.", "Add the key in Settings or <Rill home>/.env.")
		}
	}
...
func checkDirectoryMode(report *DiagnosticReport, path, label string) {
	if path == "" {
		report.add("warning", "directory.unavailable", label, label+" path is unavailable.", "Restore the user home or Rill path environment configuration.")
		return
	}
	st, err := os.Stat(path)
	if err != nil {
...
func checkDerivedJSON(report *DiagnosticReport) {
	...
	for name, path := range paths {
		...
		if !json.Valid(b) {
			report.add("warning", "derived.invalid_json", "derived:"+name, fmt.Sprintf("Derived desktop state %s is malformed.", filepath.Base(path)), "Run rill-guard rebuild --target "+name+".")
		}
	}
}
		...
		if err == nil {
			...
			if callErr != nil {
				...
				case resp.StatusCode == http.StatusUnauthorized || resp.StatusCode == http.StatusForbidden:
					report.add("error", "network.authentication_failed", "provider:"+entry.Name, fmt.Sprintf("Provider rejected credentials with HTTP %d.", resp.StatusCode), "Update the provider credential in Rill Settings.")
